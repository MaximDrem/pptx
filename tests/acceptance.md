...
node tests/unit/render-report.test.cjs      # report/artifacts after temp cleanup
node tests/unit/lint-deck.test.cjs          # //host/file: and no bypass
```
...
- **S2. Defect detection**
  ...
  `EMPTY REGION`, `BLANK SLIDE`, `BROKEN IMAGE`; exit code 0 (issues ≠ crash).
- **S3. PDF/PNG**
