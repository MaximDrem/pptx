...
| Render loop truth (v58) | reused `--out-dir` is cleaned before every render/capture (stale `slide-NN.png` made `review` print 30 pictures for a 1-slide deck), `review`/`index` print an ACTION line with the fix path on blocking errors, lint snippets are paste-ready, `LOW-CONTRAST` names the color and says to fix the color/backdrop, not the weight | ✅ |
| Loop breakers (v59) | `missing-br` names the box, both rows and says the `<br>` goes BETWEEN them (a real run put it after the last row and looped 4 cycles); `review` prints the STATIC LINT block; SKILL: **always Russian** (no "user language" flexibility), numbered slide plan, 8-or-fewer slides in one write / 9+ in two passes with `slide.cjs --append`, count check before render, decor-placement variety; `--force` exports after two honest fix attempts so a blocker cannot leave the user without a .pptx | ✅ |
...
| Artifact validation | `validate.cjs`: ports of the python checks (empty placeholders, split-box, WCAG contrast, hierarchy, fullness, density, repetition, notes) + font size, placeholders, typography, stage size, native text/fonts, normAutofit — plus a JSON report (path printed, `--json <path>`) | ✅ |
