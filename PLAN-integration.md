...
| `presentation_v3/.bundle-version` (= 47) | already inside the folder above |
...
Verify `.bundle-version` = `47` (the currently installed one = 35; when the
...
`.bundle-version = 47`. If the line is actually missing, add it following the
neighboring skills.
