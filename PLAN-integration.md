...
| `presentation_v3/.bundle-version` (= 52) | already inside the folder above |
...
Verify `.bundle-version` = `52` (the currently installed one = 35; when the
...
`.bundle-version = 52`. If the line is actually missing, add it following the
neighboring skills.
