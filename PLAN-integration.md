...
| `presentation_v3/.bundle-version` (= 50) | already inside the folder above |
...
Verify `.bundle-version` = `50` (the currently installed one = 35; when the
...
`.bundle-version = 50`. If the line is actually missing, add it following the
neighboring skills.
